...
public:
  explicit CalibratedPhotonProducerT( const int placehold ) ;
...
// template<typename T>
// CalibratedPhotonProducerT<T>::CalibratedPhotonProducerT( const int plachold) :
...
//   //  produceCalibratedObjs_(conf.getParameter<bool>("produceCalibratedObjs"),conf.getParameter<bool>("produceCalibratedObjs"))
//   // produceCalibratedObjs_()
//   //  produceCalibratedObjs_( true, conf.getParameter<bool>("produceCalibratedObjs"))
// { 
...
//     std::array<float,EGEnergySysIndex::kNrSysErrs> uncertainties = energyCorrector_.calibrate(out->back(), iEvent.id().run(), recHits, iEvent.streamID(), 0);
//     //    std::array<float,EGEnergySysIndex::kNrSysErrs> uncertainties = energyCorrector_.calibrate(out->back(), iEvent.id().run(), 12, iEvent.streamID(), 0);
//     //    std::array<float,EGEnergySysIndex::kNrSysErrs> uncertainties = energyCorrector_.calibrate(out->back(), iEvent.id().run(), 12, iEvent.streamID(), evtType);
