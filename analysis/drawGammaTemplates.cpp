...
int main( int argc, char* argv[] ) {
  ...
  compareRegions( outputdir, r_lowHT_vs_njet, templates_gJet, templates_qcd );
  ...
  compareRegions( outputdir, r_medHT_vs_njet, templates_gJet, templates_qcd );
  ...
  compareRegions( outputdir, r_highHT_vs_njet, templates_gJet, templates_qcd );
  ...
  compareRegions( outputdir, r_vsHT, templates_gJet, templates_qcd );
  ...
  compareRegions( outputdir, r_vsHT2, templates_gJet, templates_qcd );
...
void compareRegions( const std::string& outputdir, std::vector<MT2Region> regions, MT2Analysis<MT2EstimateZinvGamma>* templ, MT2Analysis<MT2EstimateZinvGamma>* templ_qcd, const std::string& suffix ) {
  ...
  for( unsigned i=0; i<regions.size(); ++i ) {
    ...
    MT2EstimateZinvGamma* thisEstimate_qcd = templ_qcd->get( regions[i] );
    if( thisEstimate==0 ) {
      std::cout << "ERROR! Didn't find estimate for region: " << regions[i].getName() << " ! Exiting." << std::endl;
      exit(119);
    }

    TH1D* thisTempl_prompt = thisEstimate->template_prompt;
  ...
  c1->SaveAs( Form("%s/templ_%s%s.eps", outputdir.c_str(), saveName.c_str(), suffix.c_str()) );
  c1->SaveAs( Form("%s/templ_%s%s.png", outputdir.c_str(), saveName.c_str(), suffix.c_str()) );

  c1_log->SaveAs( Form("%s/templ_%s%s_log.eps", outputdir.c_str(), saveName.c_str(), suffix.c_str()) );
  c1_log->SaveAs( Form("%s/templ_%s%s_log.png", outputdir.c_str(), saveName.c_str(), suffix.c_str()) );
