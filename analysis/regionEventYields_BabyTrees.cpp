...
MT2YieldAnalysis* computeYield( const MT2SampleBaby_basic& sample, std::vector<MT2HTRegion> HTRegions, std::vector<MT2SignalRegion> signalRegions );
std::vector<MT2YieldPreAnalysis*> getEventYield( std::vector<std::string> fakeID, TTree* tree, MT2SampleBaby_basic sample, std::vector<MT2HTRegion> HTRegions, std::vector<MT2SignalRegion> signalRegions );
...
int main( int argc, char* argv[] ) {
  ...
  if( argc!=2 ) {
    std::cout << "USAGE: ./regionEventYields_BabyTrees [samplesFileName]" << std::endl;
  ...
  MT2YieldAnalysis* EventYield_zjets = mergeYields( EventYield, "ZJets" );
  MT2YieldAnalysis* EventYield_other = mergeYields( EventYield, "DYJets", "GJets" );
  ...
  MT2YieldAnalysis* EventYield_bg     = new MT2YieldAnalysis(*EventYield_other);
  MT2YieldAnalysis* EventYield_allMC  = new MT2YieldAnalysis(*EventYield_top + *EventYield_wjets + *EventYield_zjets + *EventYield_qcd + *EventYield_other);
  ...
  // For top yield:
  std::vector<TH1D*> vh1_top = getYieldHistos( "EventYield_top", "", HTRegions, signalRegions, EventYield_top, EventYield_bg, logfile );

  // For W+jets yield:
  std::vector<TH1D*> vh1_wjets = getYieldHistos( "EventYield_wjets", "", HTRegions, signalRegions, EventYield_wjets, EventYield_bg, logfile );

  //For Z+jets yield:
  std::vector<TH1D*> vh1_zjets = getYieldHistos( "EventYield_zjets", "", HTRegions, signalRegions, EventYield_zjets, EventYield_bg, logfile );

  // For QCD yield:
  std::vector<TH1D*> vh1_qcd = getYieldHistos( "EventYield_qcd", "", HTRegions, signalRegions, EventYield_qcd, EventYield_bg, logfile );

  // For other SM yield:
  std::vector<TH1D*> vh1_other = getYieldHistos( "EventYield_other", "", HTRegions, signalRegions, EventYield_other, EventYield_bg, logfile );

  // For all SM yield:
  std::vector<TH1D*> vh1_mc   = getYieldHistos( "EventYield_MC", "", HTRegions, signalRegions, EventYield_allMC, EventYield_bg, logfile );
  
  // For signal yield:
  std::vector<TH1D*> vh1_signal = getYieldHistos( "EventYield_signal", "", HTRegions, signalRegions, EventYield_signal, EventYield_allMC, logfile );
