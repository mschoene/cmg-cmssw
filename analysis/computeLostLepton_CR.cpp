...
int main( int argc, char* argv[] ) {
  ...
  MT2Analysis<MT2EstimateSyst>* lostLeptonEstimate = new MT2Analysis<MT2EstimateSyst> ( "WJets", regionsSet );  
  for( unsigned i=0; i < fSamples.size(); ++i )
    (*lostLeptonEstimate) += ( computeYield( fSamples[i], regionsSet, lumi ) );
  
  lostLeptonEstimate->writeToFile("TTJets_CR_pfHad.root");
...
MT2Analysis<MT2EstimateSyst> computeYield( const MT2Sample& sample, const std::string& regionsSet, float lumi ) {
  ...
  for( unsigned iEntry=0; iEntry<nentries; ++iEntry ) {
    ...
    //if( myTree.nMuons10 > 0 && !( myTree.nElectrons10 > 0 || myTree.nPFLep5LowMT > 0 || myTree.nPFHad10LowMT > 0 ) ) ; //CR_mu
    //if( myTree.nElectrons10 > 0 && !( myTree.nMuons10 > 0 || myTree.nPFLep5LowMT > 0 || myTree.nPFHad10LowMT > 0 ) ) ; //CR_ele  
    //if( myTree.nPFLep5LowMT > 0 && !( myTree.nElectrons10 > 0 || myTree.nMuons10 > 0 || myTree.nPFHad10LowMT > 0 ) ) ; //CR_pfLep
    if( myTree.nPFHad10LowMT > 0 && !( myTree.nElectrons10 > 0 || myTree.nMuons10 > 0 || myTree.nPFLep5LowMT > 0 ) ) ; //CR_pfHad 
